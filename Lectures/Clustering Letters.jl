...
# ╔═╡ bdea5233-65ee-4803-be94-80a25cd2c33a
using Images, Clustering, Plots, LinearAlgebra, SparseArrays, Distances, Arpack
...
function plotKmeansresult(out::KmeansResult,X::AbstractArray)
    ...
    for j=1:k
        scatter!(X[1,findall(out.assignments.==j)],
            X[2,findall(out.assignments.==j)],label="Cluster $j")
    end
    # Centers
...
# ╔═╡ 83c79703-8688-4816-b661-1a7540114cfb
Laplacian(W::AbstractMatrix)=Diagonal(vec(sum(W,dims=2)))-W

# ╔═╡ c64f239f-50d2-4dc7-aa69-67c857845870
function NormalizedLaplacian(L::AbstractMatrix)
	D=inv(√Diagonal(L))
	Symmetric(D*L*D)
end

# ╔═╡ 00765f71-95bb-4d8c-ac63-6eb2d74a17a4
...
# ╔═╡ 5376e1dd-f3e2-49cf-9b68-30b628dcaee1
issymmetric(Lₙ)
